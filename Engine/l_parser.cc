...
{
    assert(get_alphabet().find(c) != get_alphabet().end());
    std::string temp = replacementrules.find(c)->second;

    std::string tempstring;
    std::vector<double> kansen;
    std::vector<std::string> tempvec;

    for (int i=0;i<temp.size();i++){
        if (isalpha(temp[i])||temp[i]=='+'||temp[i]=='('||temp[i]==')'||temp[i]=='-'||temp[i]=='&'||temp[i]=='|'||temp[i]=='^'||temp[i]=='/'||temp[i]=='\\'){
            tempstring+=temp[i];
        }
        if (temp[i]==':') {
            ...
            double kans = std::stod(tempstring);
            kansen.push_back(kans);
            tempstring = "";
        }
    }
    ...
    for(int k=0;k<tempvec.size();k++){
        if(random<=kansen[k]+stack){
            std::string* pointer=new std::string(tempvec[k]);
            return *pointer;
        }
        else{
            stack+=kansen[k];
